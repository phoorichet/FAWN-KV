...
void bytes_into_hex_string_trunc0s(const u_char *data, u_int len, string& dststr)
{
    static const char hexes[] = "0123456789ABCDEF";
    dststr.reserve(dststr.size() + len*2);
...
string bytes_to_hex(const u_char* data, u_int len)
{
    ...
    bytes_into_hex_string_trunc0s(data, len, r);
    return r;
}
...
void print_payload(const string &str) {
    print_payload((const u_char*)str.data(), str.size());
}
/*
