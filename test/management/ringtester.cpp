...
generate_random_kv(kv_array_type& out_arr, size_t key_len, size_t data_len, size_t size, unsigned int seed = 0)
{
  ...
  srand(seed);
  char* buf = new char[key_len * size];
  ...
  for (size_t i = 0; i < size; i++) {
		kv_pair kv;
   // cout << "->" << string(buf + i * kv_len, key_len) << endl;
		kv.key = silt::NewValue(buf + i * key_len, key_len);
    // cout<< "key:" << bytes_to_hex(kv.key.data()) << endl; 
		// kv.data = silt::NewValue(buf + i * kv_len + key_len, data_len);
   // cout<< "key:" << bytes_to_hex(kv.key.data()) << " value: " << bytes_to_hex(kv.data.data()) << endl ;
//    cout<< "key:" << (kv.key.size()) << " value: " << (kv.data.size()) << endl ;
...
void put_test(FrontEnd *fe, int msg_size)
{
  ...
  size_t key_size = 20;
  kv_array_type arr_;
  generate_random_kv(arr_, key_size, msg_size, maxKey, 1);
  ...
  for (uint i = 0; i < maxKey; i++) {
    ...
    // string ps_key((const char*)&num, sizeof(num));
    // uint32_t key_id = HashUtil::BobHash((const void *)ps_key.data(), ps_key.length());
//    string key = generate_random_key(key_size);
...
//    cout << bytes_to_hex(string(arr_[i].key.data(), key_size)) << endl;
    fe->put(string(arr_[i].key.data(), key_size), value, i);
...
//      fe->get(key, i);
      fe->get(string(arr_[i].key.data(), key_size), i);
    }
...
//    string key((char *)&key_id, sizeof(uint32_t));
    ...
    cout << "GET: " << string(arr_[i].key.data(), key_size) << endl;
    fe->get(string(arr_[i].key.data(), key_size), i);
